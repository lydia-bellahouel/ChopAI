...
def train(generator_model, discriminator_model, gan_model, dataset, path_discriminator_model, path_generator_model, path_gan_model, path_discriminator_info, path_images_generated_per_epoch, latent_dim = 100, n_epochs=160, n_batch_per_epoch=15):
    ...
    for i in range(n_epochs): # Enumerate epochs
        ...
        new_row = pd.DataFrame(data= [[d_loss, g_loss, accuracy_on_real, accuracy_on_fake]], columns = discriminator_info_per_epoch.columns)
        discriminator_info_per_epoch = pd.concat([discriminator_info_per_epoch, new_row], ignore_index=True)
