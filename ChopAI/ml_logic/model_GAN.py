...
# Train the GAN model, saving models and info each 10 epochs
def train(generator_model, discriminator_model, gan_model, dataset, path_discriminator_model, path_generator_model, path_gan_model, path_discriminator_info, path_images_generated_per_epoch, latent_dim = 100, n_epochs=160, n_batch_per_epoch=15):
    number_of_samples_per_batch = int((dataset.shape[0] / n_batch_per_epoch)*0.16) # Number of samples to take per batch per epoch
    ...
    # Dataframe containing loss and accuracy of discriminator for each epoch
    discriminator_info_per_epoch = pd.DataFrame(columns=['loss_discriminator_on_real_music', 'loss_discriminator_on_fake_music', 'accuracy_on_real', 'accuracy_on_fake'])
    ...
    for i in range(n_epochs): # Enumerate epochs
        ...
        discriminator_info_per_epoch = discriminator_info_per_epoch.append({'loss_discriminator_on_real_music':  d_loss, 'loss_discriminator_on_fake_music': g_loss, 'accuracy_on_real': accuracy_on_real, 'accuracy_on_fake': accuracy_on_fake}, ignore_index=True)
