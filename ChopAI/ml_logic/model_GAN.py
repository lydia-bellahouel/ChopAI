...
# Train the GAN model
def train(generator_model, discriminator_model, gan_model, dataset, latent_dim = 100, n_epochs=20, n_batch_per_epoch=15):
...
def save_discriminator(discriminator_model, output_path):
    discriminator_model.save_model(output_path, overwrite = False)
    return None

def save_generator(generator_model, output_path):
    generator_model.save_model(output_path, overwrite = False)
    return None

def save_GAN(gan_model, output_path):
    gan_model.save_model(output_path, overwrite = False)
    return None
...
def save_images_generated(output_path, images_generated_per_epoch):
    np.save(output_path, images_generated_per_epoch)
    return None
